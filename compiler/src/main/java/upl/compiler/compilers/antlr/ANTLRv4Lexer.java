	...
	public class ANTLRv4Lexer extends BaseLexer {
		...
		public enum ANTLRv4Lexeme implements Lexeme {
			...
			BEGIN_ACTION (LBrace ().pushMode (Rule.Mode.TargetLanguageAction)),
			...
			NESTED_ARGUMENT (LBrack ().setType (Rule.Action.ARGUMENT).pushMode (Rule.Mode.Argument)),
			ARGUMENT_ESCAPE (EscAny ().setType (Rule.Action.ARGUMENT)),
			ARGUMENT_STRING_LITERAL (DQuoteLiteral ().setType (Rule.Action.ARGUMENT)),
			ARGUMENT_CHAR_LITERAL (SQuoteLiteral ().setType (Rule.Action.ARGUMENT)),
			...
			NESTED_ACTION (
				LBrace ()
					.setType (Rule.Action.CONTENT)
					.pushMode (Rule.Mode.TargetLanguageAction)
			),
			
			ACTION_ESCAPE (EscAny ().setType (Rule.Action.CONTENT)),
			ACTION_STRING_LITERAL (DQuoteLiteral ().setType (Rule.Action.CONTENT)),
			ACTION_CHAR_LITERAL (SQuoteLiteral ().setType (Rule.Action.CONTENT)),
			ACTION_DOC_COMMENT (DocComment ().setType (Rule.Action.CONTENT)),
			ACTION_BLOCK_COMMENT (BlockComment ().setType (Rule.Action.CONTENT)),
			ACTION_LINE_COMMENT (LineComment ().setType (Rule.Action.CONTENT)),
			//END_ACTION (RBrace { this.handleEndAction(); }),
			UNTERMINATED_ACTION (new Rule (BaseLexeme.EOF).popMode ()),
