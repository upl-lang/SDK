	...
	import upl.regex2.tree.SyntaxTree;
	...
	public class DFA {
		...
		protected NFABitmapStateManager nfaiStateManager;
		protected List<NFABitmapState> nfaiStates;
		protected int[][] transitionTable = null;
		protected int is = -1; // init state
		protected int rs = -1; // rejected state
		protected boolean[] fs = null; // final states
		
		public DFA (String regex) {
			this (new SyntaxTree (regex));
		}
		
		public DFA (SyntaxTree tree) {
			this (new NFA (tree.getRoot ()));
		}
		
		protected DFA (NFA nfa) {
			
			nfaiStateManager = new NFABitmapStateManager (nfa.getStateList ());
			nfaiStates = nfaiStateManager.getStates ();
		...
		protected boolean dfaEquivalenceCheck (DFA other, int initialState, int otherInitialState, int[][] checked) {
			...
			for (int i = 0; i < initialTransitions.length; i++) {
				...
				if (fs[initialTransitions[i]] != other.fs[otherInitialTransitions[i]]) // one transition goes to a final state and the other does not, this DFA is not equivalent
					return false;
				...
				else if (!fs[initialTransitions[i]] && !other.fs[otherInitialTransitions[i]]) {
		...
		protected void constructOriginalDFA (NFABitmapStatePack stateSet, Map<NFABitmapState, Map<Character, NFABitmapStatePack>> nfaTransitionMap, Map<NFABitmapStatePack, Map<Character, NFABitmapStatePack>> originalDFATransitionMap) {
		...
		protected Map<NFABitmapState, NFABitmapStatePack> calculateClosure (List<NFABitmapState> nfaStateList) {
		...
		protected void dfsClosure (NFABitmapState state, NFABitmapStatePack closure) {
		...
		protected NFABitmapStatePack traceReachable (NFABitmapStatePack closure, char ch, Map<NFABitmapState, NFABitmapStatePack> closureMap) {
		...
		protected void minimize (Map<NFABitmapStatePack, Map<Character, NFABitmapStatePack>> oriDFATransitionMap, NFABitmapStatePack initClosure, NFABitmapState finalNFAIState) {
		...
		protected int[] newRejectedState () {
			int[] state = new int[CommonSets.ENCODING_LENGTH];
			rejectAll (state);
			return state;
		}
		
		protected void rejectAll (int[] state) {
