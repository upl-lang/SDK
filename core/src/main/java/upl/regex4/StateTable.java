	...
	public class StateTable {
		...
		protected State[][] stateTable = new State[20][20];
		protected Map<Integer, Integer> chMapx = new HashMap<> (); //转换条件到数组横坐标的映射
		protected Map<State, Integer> stateMapy = new HashMap<> (); //状态到数组纵坐标的映射
		protected Map<Integer, State> hashMapState = new HashMap<> (); //状态到数组纵坐标的映射
		
		protected int y = 0; //纵坐标
		protected int x = 0; //横坐标
		...
		PriorityQueue<State> getStates (int x, State y) {
			...
			if (x >= 57 && (ix = chMapx.get (Regex.DIGIT)) != null) {
			...
			if (x < 57 && (ix = chMapx.get (Regex.NODIGIT)) != null) {
			...
			if (Character.isSpaceChar (x) && (ix = chMapx.get (Regex.BLANK)) != null) {
			...
			if (!Character.isSpaceChar (x) && (ix = chMapx.get (Regex.NOBLANK)) != null) {
		...
		void addState (char x, State y, Node node0, Node node) {
			...
			} else stateTable[ix][iy].id.add (node0.id);
