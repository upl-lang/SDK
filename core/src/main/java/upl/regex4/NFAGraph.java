	...
	public class NFAGraph implements Cloneable {
		...
		protected NFAGraph cloneNFA; //克隆图时用
		protected HashMap<Node, Node> cloneMap; //克隆图时用
		...
		void repeatGraph () {
			...
			end.addNextNode (' ', node);
			
			node.addNextNode (' ', start);
		...
		void repeatGraph0 () {
			...
			end.addNextNode (' ', nfaGraph0.start);
		...
		void repeatGraph4 (int n, int m, boolean noGreed) {
			...
			for (int i = 0; i < m - n; i++) {
				...
				if (noGreed)
					this.end.addNextNode (Regex.NOGREED, end);
				else
					this.end.addNextNode (' ', end);
		...
		protected Node nodeClone (Node node) {
