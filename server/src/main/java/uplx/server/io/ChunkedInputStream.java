     ...
     */
		protected long initChunk () throws IOException {
			...
			if (limit == 0) { // finished previous chunk
				...
				BufferedReader reader = BufferedInputStream.getReader (in);
				
				if (initialized && reader.readLine ().length () > 0)
					throw new IOException ("chunk data must end with CRLF");
				
				initialized = true;
				limit = parseChunkSize (reader.readLine ()); // read next chunk size
				
				if (limit == 0) { // last chunk has size 0
					limit = -1; // mark end of stream
					// read trailing headers, if any
					Headers trailingHeaders = headers.readHeaders (reader); // TODO
					if (headers != null)
