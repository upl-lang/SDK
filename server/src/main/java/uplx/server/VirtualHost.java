	...
	import uplx.server.http.handlers.MethodHandler;
     ...
     */
		public ContextInfo getContext (String path) {
			...
			for (path = Strings.trimEnd (path, "/"); path != null; path = File.getParent (path)) {
     ...
     */
		public void addContext (String path, Handler<?, ?> handler, HttpMethod... methods) {
			...
			path = Strings.trimEnd (path, "/"); // remove trailing slash
